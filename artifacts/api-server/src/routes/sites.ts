...
export function createSiteViewRouter(): IRouter {
  ...
  viewRouter.get("/s/:name", async (req, res): Promise<void> => {
    ...
    const hideBadge = `<style>#replit-pill,#replit-badge-container,#replit-badge,.replit-badge,[data-replit-badge],[data-state="brand"],[data-state="cta"]{display:none!important;}</style><script>(function(){function remove(){document.querySelectorAll('#replit-pill,#replit-badge-container,#replit-badge,.replit-badge,[data-replit-badge],[data-state="brand"],[data-state="cta"]').forEach(function(e){e.remove();});}remove();new MutationObserver(remove).observe(document.documentElement,{childList:true,subtree:true});}());</script>`;
